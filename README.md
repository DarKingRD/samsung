...
- По умолчанию приложение ищет модель в `./models/correction_model`.
  - Модель можно скачать с нашего kaggle: [ссылка на модель](https://www.kaggle.com/code/darkingrd/rut5-base-train). Либо можно запустить обучение локально (если конечно у вас доступна cuda - иначе будете обучать модель 90+ часов 😅). Тогда достаточно выполнить следующую команду:
  ```bash
  python src/train.py
  ```
