...
class ErrorCorrectionInference:
    ...
    def _load_model_and_tokenizer(self, model_path: Optional[str]) -> None:
        ...
        try:
            if model_path and Path(model_path).exists():
                ...
                logger.info("Локальная модель не найдена. Загружаю базовую модель t5-small...")
    ...
    def correct(self, text: str, n: int = 3) -> CorrectionResult:
        ...
        if len(text) > 1000:
            logger.warning(f"Текст довольно длинный ({len(text)} символов), возможны потери при токенизации")
    ...
    def _classify_error(self, original: str, corrected: str) -> str:
        ...
        if normalize_text(original) == normalize_text(corrected) and original != corrected:
            return "punctuation"
    ...
    def _find_corrections(self, original: str, corrected: str) -> List[Correction]:
        ...
        for operation_type, orig_start, orig_end, corr_start, corr_end in matcher.get_opcodes():
    ...
    def highlight_errors(self, text: str, variant_index: int = 0) -> str:
        ...
        if variant_index >= len(result.variants):
            raise IndexError(f"Вариант {variant_index} не существует. Доступно {len(result.variants)} вариантов.")
        ...
        for correction in sorted(variant.corrections, key=lambda c: c.position):
            ...
            error_part = highlighted_text[position:position + len(correction.original)]
...
if __name__ == "__main__":
    ...
    test_text = "Привет как дела? Меня зовут Андрей, я живу в Москве. " \
