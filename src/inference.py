...
class TypoCorrectionInference:
    ...
    def _correct_with_model(self, text: str, top_k: int) -> List[Tuple[str, float]]:
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                max_length=512,
                num_beams=5,
                num_return_sequences=min(top_k, 5),
                early_stopping=True,
                temperature=0.7,
            )
        ...
        return corrections if corrections else [(text, 1.0)]
    
    def _is_valid_word(self, word: str) -> bool:
        """Проверяет, является ли слово правильным (существует в языке)."""
        if not word or len(word) < 2:
            return True  # Короткие слова пропускаем
        
        word_lower = word.lower().strip()
        
        # Если слово короткое (1-3 символа), считаем его правильным
        if len(word_lower) <= 3:
            return True
        
        # Проверяем через морфологический анализатор (если доступен)
        if self.morph:
            try:
                parsed = self.morph.parse(word_lower)
                if parsed:
                    # Если слово имеет нормальную форму (не опечатка)
                    # и вероятность > 0.3, считаем его правильным
                    for p in parsed:
                        if p.score > 0.3:  # Повышаем порог для более строгой проверки
                            return True
            except:
                pass
        
        # Проверяем в словаре правильных слов (из словаря опечаток)
        if word_lower in self.correct_words:
            return True
        
        # Если слово не найдено в словаре опечаток - считаем его правильным
        # (чтобы не исправлять все неизвестные слова)
        # Это основная логика: исправляем только те слова, которые точно есть в словаре опечаток
        if word_lower not in self.typo_dict:
            return True
        
        # Если слово есть в словаре опечаток, но нет в словаре правильных слов
        # и морфологический анализатор недоступен - считаем опечаткой
        # (это безопаснее, чем не исправлять)
        return False
    ...
    def _correct_with_dict_and_rules(self, text: str, top_k: int) -> List[Tuple[str, float]]:
        ...
        for word in words:
            ...
            if word_lower in self.typo_dict:
                ...
                similar_correction, confidence = self._find_similar_word(word, max_distance=2)
    ...
    def _apply_grammar_rules(self, text: str) -> str:
        ...
        text = re.sub(r'(\w+)\s+(что|который|которая|которое|которые|когда|где|куда|откуда)\s+', 
                     r'\1, \2 ', text)
        ...
        text = re.sub(r'(\w+)\s+(который|которая|которое|которые)\s+', 
