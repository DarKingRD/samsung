...
class TypoCorrectionModel(nn.Module):
    ...
    def __init__(self, model_name: str = "DeepPavlov/rubert-base-cased", use_seq2seq: bool = False):
        super().__init__()
    ...
    def _bert_mlm_corrections(self, text: str, inputs: dict, top_k: int) -> List[Tuple[str, float]]:
...
class SimpleTypoCorrector:
    ...
    def correct(self, text: str) -> List[Tuple[str, float]]:
        ...
        import re
        corrected = re.sub(r'(\w+)\s+(что|который|когда|где|куда)\s+', r'\1, \2 ', corrected)
